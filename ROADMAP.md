...
> 已实现 / 已修复的内容不在此流水账，见 [`docs/FEATURES.md`](docs/FEATURES.md)；
> 逐条待办的**唯一来源**仍是 [`docs/todolist.md`](docs/todolist.md)（战术层）；
...
> P0 / P1 的逐条修复记录与验证证据见 [`docs/FEATURES.md`](docs/FEATURES.md)「H」「I」段，
> 不在此重复。本文件只列**未完成**项。
...
| # | 条目 | 来源 | 状态 | 验收标准 |
|---|---|---|---|---|
| 4 | ZIP 部分失败可见（S6） | ASSESSMENT S6 / todolist #20 | ⬜ | ZIP 打包中单 key 失败需有服务端日志与指标（当前仅写入包内 `_下载失败清单.txt`，handler 忽略 `failKeys`，无法观测失败率）。**已完成**：`copyStream` 返回并记录 error + `s3c_stream_interrupted` 计数 |
...
| # | 条目 | 来源 | 状态 | 验收标准 |
|---|---|---|---|---|
| 7 | grid 视图窗口化 + `ObjectList` 清理 | ASSESSMENT D8 / todolist #11 | ⬜ | grid `v-for` 加渲染上限或窗口化；移除未使用的 `visibleCount` prop（声明于 `ObjectList.vue:23`，模板未引用） |
| 8 | 前端健康轮询与自动恢复 | ASSESSMENT S5/S8 / todolist #22 | ⬜ | 后端不可用后自动重试 / 恢复；`useBucketSetting.reload()` 加 seq 或 AbortController 竞态守卫 |
| 9 | 后端死代码清理（收尾） | ASSESSMENT D5/D6 / todolist #9-10 | ⏳ | **已完成**：`ctxReader` / `batchItemError` / `Client.S3()` 已删。**剩余**：① endpoint 归一化两份实现行为不一致（`s3wrap/client.go` 对 `HTTP://Host` 产出损坏的 `http://HTTP://Host`，`service/migrate.go` 版本正确）需合并为单一 helper；② `derefString`/`derefInt64`/`timeOrZero`/`boolOrFalse` 在 `handler` 侧**生产零引用**（仅 `acc_core_test.go` 引用，属覆盖率注水）应删，`s3wrap` 侧跨 `helpers.go`/`s3wrap_dto.go` 的重复应去重 |
| 10 | i18n 死键清理与重复排序 | ASSESSMENT D7/D9 / todolist #11 | ⬜ | 清理约 40 个未引用键（保留 `storage.class.*` / `provider.*` 等动态模板键）；`entries` / `visibleEntries` 单次排序 |
...
| Go 覆盖率 | `go test -coverprofile` 总 statements | ✅ 99.6%（handler 99.8% / service 97.8% / 其余 100%）；CI 阈值 90% |
...
> 后端覆盖率未达 100%（handler / service 存在少量 gap），CI 以 **90%** 为硬阈值；
> 「各包 100%」的旧表述已按实测修正。去注水见 #11。
