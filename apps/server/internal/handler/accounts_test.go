...
func TestHealthStoreUnavailable(t *testing.T) {
	...
	h := New(&failPingStore{}, logger, t.TempDir(), nil, "", "test", false, false).Routes()
	rr := doJSON(t, h, "GET", "/api/health", "")
...
func TestSecurityHeaders(t *testing.T) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	h2 := New(st, logger, dir, nil, "", "test", false, false).Routes()
...
func TestSPAFallback(t *testing.T) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	h := New(st, logger, dir, nil, "", "test", false, false).Routes()
