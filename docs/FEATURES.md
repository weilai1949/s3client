...
| # | 项 | 状态 | 修复内容 |
|---|----|------|----------|
| P1-1 | Go 1.26.5 的 6 个可达 stdlib 漏洞（H2） | ✅ | `server/go.mod` 与 `server/Dockerfile` 升至 1.26.6（CI 经 `go-version-file` 自动跟随）。CI 在 `go vet` 后新增固定版本 `govulncheck@v1.8.0` 门禁，补上 Trivy 只扫 OS/库、拦不住标准库 CVE 的盲区 |
