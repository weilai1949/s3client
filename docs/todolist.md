...
| # | 项 | 来源 | 状态 | 说明 |
|---|----|------|------|------|
| 9 | ~~后端死代码：`ctxReader`、`batchItemError`、`Client.S3()`~~、`isNoSuchBucketSetting` | ASSESSMENT D1-D4 | ✅ | 已删三项：`ctxReader`（与 `ctxCancelReader` 职责重复且零生产引用）、`batchItemError`（生产零引用，唯一格式串在 `service/batch.go` 内联）、`Client.S3()`（导出零生产调用，E2E 清理改用已有的 `DeleteObjectVersion`/`DeleteObject`）。**`isNoSuchBucketSetting` 经核验有 5 处生产调用，并非死代码**——原条目描述有误，故保留 |
