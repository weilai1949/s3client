...
| XSS | 全库 0 处 `v-html`/`innerHTML`/`eval`；预览全走服务端代理 + sandbox + 类型白名单 | `web/src` 全量 grep |
...
| **H4** | **前端 Token 明文双份落 localStorage**：`ServerProfile.token` 随 `s3c.servers` 整体明文写入 localStorage，旁路了代码自己声明的「token 仅 sessionStorage」安全策略 | `web/src/api.ts:143-151,213-244` | 任何 XSS 或同源恶意脚本可持久窃取全部后端 token |
...
| # | 死代码 | 证据 |
|---|---|---|
| D7 | 约 40 个 i18n 键「定义但从未引用」（`common.back/confirm/...`、`toolbar.delete/...`、`trash.empty` 等；已排除 `storage.class.*`/`provider.*`/`policy.tpl*` 动态模板键） | `i18n/messages/*.ts` 全量扫描 |
| D8 | `ObjectList.vue` 的 `visibleCount` prop 声明但模板不使用；grid 视图 `v-for` 全量渲染未窗口化（list 有虚拟滚动） | `ObjectList.vue:23,113-135` |
| D9 | `entries` 与 `visibleEntries` 重复排序（O(n log n)×2） | `useObjectBrowser.ts:81-115` |
| D10 | `@vitest/coverage-istanbul` 与 `coverage-v8` 双装冗余 | `web/package.json` |
...
- **CI 门禁完备**：gofmt/vet/golangci-lint/race/覆盖率（后端 90%、前端 100%）/Docker 构建/Trivy/E2E×2/桌面 cargo check，actions 全部 pin SHA（除 H3）。
...
12. **D1-D10** 死代码清理（后端 6 处 + 前端 i18n 死键 + grid 窗口化）
13. **M6** TLS 前置补 HSTS
...
- **深度审查子代理**：后端 Go（27 生产文件 + 78 测试文件）、前端 Vue/TS（web/src 全量）、安全威胁模型（STRIDE × 5 边界 + govulncheck/pnpm audit 实跑）、SRE 可靠性（降级/可观测性/CI）。
